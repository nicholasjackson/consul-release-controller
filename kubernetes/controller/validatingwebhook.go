...
import (
	...
	admissionPlugin "github.com/nicholasjackson/consul-release-controller/controller"
...
type deploymentAdmission struct {
	...
	admission admissionPlugin.Admission
}

func NewDeploymentAdmission(client client.Client, admission admissionPlugin.Admission) *deploymentAdmission {
...
func (a *deploymentAdmission) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	if err != nil && resp == admissionPlugin.AdmissionError {
		return admission.Errored(http.StatusInternalServerError, err)
	}

	if resp == admissionPlugin.AdmissionGranted {
